...
function ArticleList(props) {
  ...
  const delBBDAction = id => {
    console.log('delBBDAction', id)
    confirm({
      title: '确定要删除这个视频吗?',
      content: '如果你点击OK按钮，视频将永远被删除，无法恢复。',
      onOk() {
        axios(servicePath.delBBDbyId + id, { withCredentials: true }).then(res => {
  ...
  const getList = () => {
    axios({
      ...
      console.log('getList', res.data.list)
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <Drawer title="Add New Video" plaement="right" closable="false" width={400} onClose={onClose} visible={visible}>
          <div>
            <div style={{ marginBottom: 16 }}>
              <Input size="large" value={title} onChange={changeTitle} placeholder="视频标题" />
            </div>
            <div style={{ marginBottom: 16 }}>
              <Input size="large" value={videoUrl} onChange={changeVideoUrl} placeholder="视频地址" />
            </div>
            <div style={{ marginBottom: 16 }}>
              <TextArea rows={3} value={imageUrl} onChange={changeImageUrl} placeholder="图片地址" />
            </div>
            <div style={{ marginBottom: 16 }}>
